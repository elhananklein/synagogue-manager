...
import { isPhoneBrowser, isTvUserAgent, isWallSizedScreen, wantsWallPreview } from "@/lib/handheld";
...
export function PersistDisplaySynagogueCookie({ synagogueId }: { synagogueId: string | null }) {
  useEffect(() => {
    ...
    const lockWall =
      !wantsWallPreview() && (!isPhoneBrowser() || isWallSizedScreen() || isTvUserAgent(ua));
    if (lockWall) {
